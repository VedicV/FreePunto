...
enum StatusIconFactory {
    ...
    private static func keyIcon(topRight: TopRight, fixedMode: Bool) -> NSImage {
        ...
        let image = NSImage(size: size, flipped: false) { rect in
            ...
            NSAttributedString(string: "⌃", attributes: controlAttributes).draw(at: NSPoint(x: 4.2, y: 1.6))
            ...
            case let .text(text):
                ...
                label.draw(at: NSPoint(x: 20.0 - textSize.width, y: 6.8))
            case .pause:
                NSColor.black.setFill()
                NSBezierPath(roundedRect: NSRect(x: 14.0, y: 7.0, width: 2.0, height: 5.6), xRadius: 0.6, yRadius: 0.6).fill()
                NSBezierPath(roundedRect: NSRect(x: 17.0, y: 7.0, width: 2.0, height: 5.6), xRadius: 0.6, yRadius: 0.6).fill()
            }

            if fixedMode {
                NSColor.black.setFill()
                NSBezierPath(ovalIn: NSRect(x: 4.0, y: 11.5, width: 2.2, height: 2.2)).fill()
            }
